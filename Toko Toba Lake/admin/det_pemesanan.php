...
while($d=mysqli_fetch_array($det)){
	...
	<table class="table">
		...
		<?php
			$user = mysqli_query($koneksi, "SELECT * FROM pemesanan WHERE id_pemesanan = '$id_pemesanan'") or die(mysqli_error($koneksi));
			$u = mysqli_fetch_assoc($user);
		?>
			<tr>
			<td>Alamat</td>
		...
		<tr>
			<td></td>
				<td><a class="btn btn-primary" href="confirm_barang_terkirim.php?id=<?= $d['id_pemesanan']?>"> Konfirmasi </a>
				<a class="btn btn-danger" href="confirm_barang_ditolak.php?id=<?= $d['id_pemesanan']?>">Reject</a></td>
		<tr/>
		<?php
			}
		?>
